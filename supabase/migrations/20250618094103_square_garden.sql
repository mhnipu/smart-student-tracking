/*
  # Enhanced Student Performance Tracker Features - Safe Migration

  This migration safely adds new features to existing database.
  It uses IF NOT EXISTS and IF EXISTS checks to avoid conflicts.

  1. New Tables (only if they don't exist)
    ...
    - `suggestions` - AI-powered study suggestions
    - `flashcards` - Digital flashcard system
    - `study_plans` - Structured study planning
    - `ai_insights` - AI-generated insights
    - `progress_snapshots` - Historical tracking
    - `resource_library` - Learning resources
...
BEGIN
  ...
  END IF;
  IF EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'resource_library') THEN
    ALTER TABLE resource_library ENABLE ROW LEVEL SECURITY;
  END IF;
  IF EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'user_resources') THEN
  ...
  END IF;
  IF EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'improvement_plans') THEN
    ALTER TABLE improvement_plans ENABLE ROW LEVEL SECURITY;
  END IF;
  IF EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'performance_logs') THEN
    ALTER TABLE performance_logs ENABLE ROW LEVEL SECURITY;
  END IF;
  IF EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'collaboration_groups') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'suggestions' AND policyname = 'Users can manage own suggestions') THEN
    CREATE POLICY "Users can manage own suggestions" ON suggestions FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ai_insights' AND policyname = 'Users can manage own AI insights') THEN
    CREATE POLICY "Users can manage own AI insights" ON ai_insights FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'progress_snapshots' AND policyname = 'Users can view own progress snapshots') THEN
    CREATE POLICY "Users can view own progress snapshots" ON progress_snapshots FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'resource_library' AND policyname = 'Anyone can view resource library') THEN
    CREATE POLICY "Anyone can view resource library" ON resource_library FOR SELECT TO authenticated USING (true);
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'improvement_plans' AND policyname = 'Users can manage own improvement plans') THEN
    CREATE POLICY "Users can manage own improvement plans" ON improvement_plans FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'performance_logs' AND policyname = 'Users can manage own performance logs') THEN
    CREATE POLICY "Users can manage own performance logs" ON performance_logs FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'collaboration_groups' AND policyname = 'Users can manage collaboration groups') THEN
    CREATE POLICY "Users can manage collaboration groups" ON collaboration_groups FOR ALL TO authenticated USING (
      id IN (
        SELECT group_id
